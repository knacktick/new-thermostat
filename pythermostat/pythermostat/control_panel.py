...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, args):
        ...
        for ch in range(self.NUM_CHANNELS):
            ...
            self._settings_tree_view.sigQueuedChangedSetting.connect(self._show_apply_all_settings_btn)
    ...
    async def _apply_all_settings_btn_clicked(self, ch, clicked):
        self._queued_changes = self._settings_tree_view.queued_changes
        ...
        for param in self._queued_changes:
            ...
            if not "pid_autotune" in param.opts:
                await self._settings_tree_view.apply_setting(param, ch, data, thermostat_param)
            else:
                self._settings_tree_view.autotuners.set_params(param.opts["pid_autotune"], ch, param)
            
        self._settings_tree_view.flush_queued_settings()
        self.apply_all_settings_btns[ch].setVisible(False)
